...
module Phylo.NLOpt (bobyqa) where
...
bobyqa :: Double -> [Double] -> ([Double] -> Double) -> [Maybe Double] -> [Maybe Double] -> IO ([Double],Int)


bobyqa xtol params f lower upper = do lower' <- newArray $ map (realToFrac . fromMaybe (-1E100)) lower
                                      ...
                                      let retCode = fromIntegral $ bobyqa_ (realToFrac xtol) startP (fromIntegral np) f'' lower' upper' 
